...
**这里以在Ubuntu中安装`gcc/g++`版本15为例。这个版本支持最新（但不是全部）的`C23/C++23`标准。**

>注意：这要求Ubuntu版本是25.04及以上。如果版本较低，那么请先升级Ubuntu内核至少到25.04。如果暂时不能，那么可以暂时使用gcc/g++版本14。
...
1. 卸载旧版本
    ...
    如果显示的主版本号低于**15**（例如是14），那么先卸载它：
    ```bash
    sudo apt remove gcc-14 g++-14 -y
    ```
...
2. 安装最新版
    ...
    在终端中发出如下命令：
    ```bash
    sudo apt install gcc-15 g++-15 -y
    ```

    >注：如果你的Ubuntu版本是26.04，那么可以安装gcc-16，即把命令中的-15改为-16。后面的命令也这样。
...
3. 为gcc/g++命令建立软链接
    ...
    **注：这些命令需要一条一条发出。每发出一条命令后按回车，命令成功执行后再发第二条命令。**
    ```bash
    cd /usr/local/bin
    sudo ln -fs /usr/bin/gcc-15 gcc
    sudo ln -fs /usr/bin/g++-15 g++
    ```
...
4. 验证是否设置成功
    ...
    g++ -v
    ```
    如果看到显示的最后一行中，主版本号为15，那么设置就成功了。

    **如果你想使用clang编译器，那么请将上述步骤中的`gcc`改为`clang`，`g++`改为`clang++`；`gcc-15`改为`clang-21`，`g++-15`改为`clang++-21`。**

    >注：如果你的Ubuntu版本是26.04，那么可以安装clang-22。
...
2. 如果`gcc`或`g++`不存在
    ...
    ```bash
    cd /usr/local/bin
    sudo ln -fs /usr/bin/clang gcc
    sudo ln -fs /usr/bin/clang++ g++
    ```
