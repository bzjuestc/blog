...
本文以**Ubuntu 25.10**为例。macOS用户请忽略此文档。
...
Ubuntu系统更新需要一个更新源配置目录**sources.list.d**，里面记录的默认的服务器是Ubuntu的主站，因其部署在国外，从而导致下载速度会比较慢。为加速下载，需要将服务器改为国内的镜像，例如清华大学的**tuna镜像网站**。
...
2. 在终端中发出如下命令：
    ...
    **注：这些命令需要一条一条发出。每发出一条命令后按回车，命令成功执行后再发第二条命令。**
    ...
    ```
    在`vim`编辑器中，使用向下键将光标移动到编辑器底部，你看到的内容可能是这样的：
    ...
    Signed-By: /usr/share/keyrings/ubuntu-archive-keyring.gpg
    ```
    按`i`键进入插入模式，将内容修改为如下：
    - 在URIs中，将`http://archive.ubuntu.com/ubuntu`修改为`https://mirrors.tuna.tsinghua.edu.cn/ubuntu`。
    - 在Suites中，在此行的最后添加`questing-security`。
    - 注释掉最后5行，即在每行前添加`#`。
    >注：你需要将版本代号（例如此例中的questing）替换为实际的版本代号。
    ...
    >注：除了清华的tuna镜像， 你还可以使用**阿里云镜像**、**腾讯云镜像**等等。
...
>目前Ubuntu最新版本是26.04 LTS，代号resolute。
